const Form = {template:
	...
	methods:{ 
					...
					ary.then((value) => {
						...
						if(value.length > 0){
							for(let i = 0; i < loop; i++){
								txt += '<li><a href="/explore/' +value[i].item.url +'">'+ value[i].item.city +'<br/><span> ' + value[i].item.admin_name + ', ' + value[i].item.country + '</span></a></li>';
							}
		...
		sendQuery: function(event){
			...
			if(txt != ''){window.location.href = '/r?q=' + txt;}
		}
	}
};
