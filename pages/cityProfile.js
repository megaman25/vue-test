const CityProfile = {
		...
		`<div class="section spacer-bottom-2x">
			...
			<div class="content-margin" style="max-width:1136px;">
				...
				<div class="box margin-bottom-50" style="color:#333;">
					<time style="font-size:200px; line-height:1.2em;">{{ TargetTime }}</time>
				</div>

				<div class="box margin-bottom-50" style="color:#333;">
					<time style="font-size:50px; line-height:1.2em;">{{ LocalTime }}</time>
				</div>
	...
	mounted(){
		...
		this.updateTime = ()=>{
				let d = new Date();
				this.LocalTime = `${hrs(d.getHours(),12)}:${z(d.getMinutes())}:${z(d.getSeconds())} ${meridian('12')}`;
			}
		...
		this.updateWorldTime = ()=>{
				...
				this.TargetTime = `${hrs(W.hour,12)}:${W.minute}:${W.second} ${meridianWorld(W.hour)}`;
			}
